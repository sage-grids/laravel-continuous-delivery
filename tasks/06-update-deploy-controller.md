# Task 06: Update Deploy Controller
...
Refactor `DeployController` to handle both push events (staging) and release events (production), creating deployment records and dispatching jobs appropriately.

---

## File: `src/Http/Controllers/DeployController.php`
...
class DeployController extends Controller
{
    ...
    protected function verifySignature(Request $request): bool
    {
        ...
        return Signature::verifyGithubSha256($signature, $payload, $secret);
    }
...
1. **Dual Event Handling:** Processes both `push` and `release` GitHub events
2. **Environment Matching:** Maps branches to staging, tags to production
3. **Conflict Prevention:** Rejects new deployments if one is already active
4. **Approval Flow:** Creates pending deployment for production
5. **Auto-Deploy:** Immediately queues staging deployments

---

## Acceptance Criteria

- [ ] Push to `develop` creates staging deployment
- [ ] Release tag creates production deployment with pending approval
- [ ] Invalid signatures are rejected with 403
- [ ] Repository filtering works when configured
- [ ] Concurrent deployments to same environment are blocked
- [ ] Notifications are sent (with graceful failure handling)
...
- Uses `match` expression for cleaner event handling
- Notification failures are logged but don't block deployment
- Status endpoint uses UUID for public-facing lookups
